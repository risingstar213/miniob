...
InsertStmt::InsertStmt(Table *table, std::vector<std::vector<Value>> &rows)
    : table_(table)
{
  rows_.swap(rows);
}
...
RC InsertStmt::create(Db *db, const InsertSqlNode &inserts, Stmt *&stmt)
{
  ...
  // check fields type
  for (int j = 0; j < inserts.rows.size(); j++) {

    // check the fields number
    auto &values = inserts.rows[j].values;
    const int value_num = static_cast<int>(inserts.rows[j].values.size());
    ...
    rows.emplace_back(new_values);
  }

  // everything alright
  stmt = new InsertStmt(table, rows);
  return RC::SUCCESS;
}
