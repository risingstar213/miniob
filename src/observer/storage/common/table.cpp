...
RC Table::create_index(Trx *trx, const char *index_name, char *attribute_name[], int attribute_num, bool is_unique)
{
  ...
  std::vector<std::string> attribute_name_vector;
  for (int i = 0; i < attribute_num; i++) {
    attribute_name_vector.push_back(attribute_name[i]);
  }
  if (table_meta_.index(index_name) != nullptr || table_meta_.find_index_by_field(attribute_name_vector)) {
    LOG_INFO("Invalid input arguments, table name is %s, index %s exist or attributes exist index",
        name(),
        index_name);
...
IndexScanner *Table::find_index_for_scan(const DefaultConditionFilter &filter)
{
  ...
  field_name.push_back(field_meta->name());
...
RC Table::update_record(Trx *trx, Record *old_record, int cell_num, Value *values)
{
  RC rc = RC::SUCCESS;
